...
def render_summary_markdown(
    ...
    lines.append("")
    lines.append("**统计**")
    lines.append(f"- 候选样本: **{total}**")
    lines.append(f"- 候选池(命中≥2): **{candidates}**")
    ...
    if focus:
        ...
        lines.append("")
        lines.append("| 代码 | 名称 | 涨跌幅 | 成交额(亿) | 命中 | 评分 | 理由 |")
        lines.append("| --- | --- | --- | --- | --- | --- | --- |")
        for e in focus:
            s, a = e.snapshot, e.ai
            reason = (a.reason or "").replace("|", "/")[:40]
            lines.append(
                f"| {s.code} | {s.name} | {s.pct_change:.2f}% | {s.turnover/1e8:.2f} | {e.strategy.hits} | {a.score} | {reason} |"
            )
    else:
        lines.append("")
